...
class WardLevelProjectSerializer(serializers.ModelSerializer):
    ...
    project_level_id = serializers.PrimaryKeyRelatedField(
        queryset=ProjectLevel.objects.all(), source='project_level', write_only=True, allow_null=True, required=False
    )
    ...
    expenditure_title_id = serializers.PrimaryKeyRelatedField(
        queryset=ExpenditureTitle.objects.all(), source='expenditure_title', write_only=True, allow_null=True, required=False
    )

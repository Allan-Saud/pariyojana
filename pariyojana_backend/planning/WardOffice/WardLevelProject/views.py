...
class WardLevelProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ward_no = serializer.validated_data.get('ward_no')
        # Get the max priority number for this ward
        max_priority = WardLevelProject.objects.filter(
            ward_no=ward_no,
            is_deleted=False
        ).aggregate(models.Max('priority_no'))['priority_no__max'] or 0
        
        # If priority_no wasn't provided in the request, set it to max + 1
        if 'priority_no' not in serializer.validated_data:
            serializer.validated_data['priority_no'] = max_priority + 1

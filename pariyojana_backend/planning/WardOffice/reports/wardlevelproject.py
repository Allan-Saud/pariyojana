...
class DoughnutChartData(APIView):
    def get(self, request):
        # Query all active projects
        projects = WardLevelProject.objects.filter(is_deleted=False)
...
class WardLevelProjectDownloadReport(APIView):
    ...
    def generate_excel(self, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=municipality_report.xlsx'
        return response
    ...
    def generate_pdf(self, queryset):
        ...
        if not pdf.err:
            return FileResponse(result, content_type='application/pdf', filename='municipality_report.pdf')
        else:

...
class PrioritizedWardLevelProject(models.Model):
    plan_entry = models.ForeignKey(PlanEntry, on_delete=models.CASCADE, related_name="prioritized_ward_projects", null=True, blank=True)
    ...
    unit = models.ForeignKey('project_settings.Unit', on_delete=models.PROTECT, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.priority_no and not self.pk:
            max_priority = PrioritizedWardLevelProject.objects.filter(

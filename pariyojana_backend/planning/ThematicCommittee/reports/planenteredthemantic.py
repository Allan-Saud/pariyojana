...
class PlanEnteredByThematicCommitteeDownloadReport(APIView):
    def get_queryset(self, request):
        queryset = PlanEnteredByThematicCommittee.objects.filter(is_deleted=False)
    ...
    def generate_excel(self, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=plan_entered_thematic.xlsx'
        return response
    ...
    def generate_pdf(self, queryset):
        ...
        if not pdf.err:
            return FileResponse(result, content_type='application/pdf', filename='plan_entered_thematic.pdf')
        else:

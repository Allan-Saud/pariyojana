...
class WardRecommendedProjectsDownloadReport(APIView):
    def get_queryset(self, request):
        queryset = WardRecommendedProjects.objects.filter(is_deleted=False)
    ...
    def generate_excel(self, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=ward_recommended.xlsx'
        return response
    ...
    def generate_pdf(self, queryset):
        ...
        if not pdf.err:
            return FileResponse(result, content_type='application/pdf', filename='ward_recommended.pdf')
        else:

...
class MapCostEstimate(models.Model):
    ...
    project = models.ForeignKey('projects.Project', on_delete=models.CASCADE, related_name='map_cost_estimates')
    title = models.CharField(max_length=255, choices=DOCUMENT_CHOICES)
    date = models.DateField(default=timezone.localdate)
    file = models.FileField(upload_to='map_cost_estimates/', null=True, blank=True)
    remarks = models.TextField(null=True, blank=True)
    ...
    checker = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='checked_docs')
    approver = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_docs')
